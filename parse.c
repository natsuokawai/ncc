...
static Node *return_stmt(Token **rest, Token *tok) {
    ...
    Node *node = new_unary(ND_RET_STMT, expr(&tok, tok));
    *rest = skip(tok, ";");
    return node;
}
...
static Node *relation(Token **rest, Token *tok) {
    ...
    for (;;) {
        ...
        if (equal(tok, ">=")) {
            node = new_binary(ND_LE, relation(&tok, tok->next), node);
            continue;
        }

        if (equal(tok, ">")) {
            node = new_binary(ND_LT, relation(&tok, tok->next), node);
            continue;
        }
